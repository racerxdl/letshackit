---
title: "GOES Satellite Hunt (Part 3 – Frame Decoder) | Lets Hack It"
description: "Master the GOES satellite frame decoder! This guide explains convolution encoding, Viterbi decoding, and CCSDS packet synchronization for LRIT signals."
image: "https://lucasteske.dev/wp-content/uploads/2016/11/qqk3UwA-624x135.png"
---

# GOES Satellite Hunt (Part 3 – Frame Decoder)

 ![GOES Satellite Hunt (Part 3 – Frame Decoder)](/wp-content/uploads/2016/11/qqk3UwA-624x135.png)Written by Lucas Teske   
on 2 November 2016
...
[![Convolutional code trellis diagram.svg](https://upload.wikimedia.org/wikipedia/commons/thumb/a/a0/Convolutional_code_trellis_diagram.svg/1200px-Convolutional_code_trellis_diagram.svg.png)](https://commons.wikimedia.org/wiki/File:Convolutional_code_trellis_diagram.svg#/media/File:Convolutional_code_trellis_diagram.svg)
...
```
[![Convolutional encoder non-recursive.png](https://upload.wikimedia.org/wikipedia/commons/2/21/Convolutional_encoder_non-recursive.png)](https://commons.wikimedia.org/wiki/File:Convolutional_encoder_non-recursive.png#/media/File:Convolutional_encoder_non-recursive.png)By[Teridon](https://en.wikipedia.org/wiki/User:Teridon "wikipedia:User:Teridon")at[English Wikipedia](https://en.wikipedia.org/wiki/ "wikipedia:")-Own work([Teridon](https://en.wikipedia.org/wiki/User:Teridon "en:User:Teridon")), Public Domain,[Link](https://commons.wikimedia.org/w/index.php?curid=12803371)
```
...
int main(int argc,char *argv[]) {
  ...
  while(processedBytes < wordSize){
    ...
    for(int i=7;i>=0;i--){
      ...
      // Encoded Word
      encodedWord |= \(uint64\_t)\(1-parity\(encstate & polys\[0])) << \(encodedWordPosition);
      encodedWord |= \(uint64\_t)\(1-parity\(encstate & polys\[1])) << \(encodedWordPosition-1);
...
With this piece of code, you can run&nbsp;**checkCorrelation(buffer, length, corr)** and get the correlation statistics. In the&nbsp; **corr** object you will have the fields&nbsp; **uw0p** and&nbsp; **uw2p** with the positions of the highest correlation of the buffer and in&nbsp; **uw0mc** and&nbsp; **uw2mc** you will have the correlation of that position. With this two values you can know the position that your sync word is, and what is the phase shift that the Costas Loop locked into (if uw0 is 0 degrees, if uw2 is 180 degrees). To correct the data if the output is 180 degrees phase shifted, just invert every bit in your sequence. I do that by XOR’ing every byte with **0xFF**. Now if you output this to a file and inspect with [BitDisplay](https://github.com/racerxdl/open-satellite-project/tree/master/Toolset/BitDisplay)&nbsp;you will see a pattern like this:
 ...
 ![](https://i.imgur.com/pDE0R9Wh.png)
...
Then in the variable **signalErrors** you will have the percent of wrong bits that had been corrected in your frame output. After that if you output to a file and check with BitDisplay you will see something like this:
 ...
 ![](https://i.imgur.com/IUd3ItLh.png)
...
Lucas Teske. “GOES Satellite Hunt (Part 3 – Frame Decoder).” _Lets Hack It_, 2016. [https://lucasteske.dev/2016/11/goes-satellite-hunt-part-3-frame-decoder/](https://lucasteske.dev/2016/11/goes-satellite-hunt-part-3-frame-decoder/).

### BibTeX

```
@misc{teske2016goessatellitehuntpart3framedecoder,
  author = {Lucas Teske},
  title = {GOES Satellite Hunt (Part 3 – Frame Decoder)},
  year = {2016},
  publisher = {Lets Hack It},
  url = {https://lucasteske.dev/2016/11/goes-satellite-hunt-part-3-frame-decoder/}
}
```
