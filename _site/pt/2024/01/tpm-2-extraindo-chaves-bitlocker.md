---
title: "TPM 2.0: Extrair chaves do Bitlocker por SPI | Lets Hack It"
description: "Aprenda a extrair chaves do BitLocker via TPM 2.0 e SPI. Guia completo de hardware hacking e engenharia reversa de segurança."
image: "https://lucasteske.dev/assets/gepeto/tpm2.0.jpg"
---

# TPM 2.0: Extrair chaves do Bitlocker por SPI

 ![TPM 2.0: Extrair chaves do Bitlocker por SPI](/assets/gepeto/tpm2.0.jpg)Escrito por Lucas Teske   
em 16 Janeiro 2024
...
No pulseview, podemos analisar os bytes SPI usando a função decodificadora de SPI. ![Red screwdriver icon, two hexagonal shapes, and a function graph icon in a UI interface.](/assets/posts/patreon/Pasted%20image%2020230124035010.png) ![SPI decoder selector interface highlighting "Serial Peripheral Interface" option in a list.](/assets/posts/patreon/Pasted%20image%2020230124035020.png)

Em seguida, precisamos configurar o decodificador SPI para os sinais corretos para poder ver os bytes decodificados. ![SPI configuration panel with green color, showing clock, data, and chip-select settings.](/assets/posts/patreon/Pasted%20image%2020230124035029.png)

Então, você notará que, como deixamos o campo de polaridade do CS# como “ **active-low** ”, ele está decodificando apenas as transações do BIOS Flash, não o “resto” do barramento. ![SPI bus timing diagram showing CS, DO, DI, CLK signals and MISO/MOSI data transfers.](/assets/posts/patreon/Pasted%20image%2020230124035036.png)

Se selecionarmos “ **active-high** ”, analisaremos qualquer coisa que **não** seja o BIOS flash (que é o que queremos): ![SPI bus timing diagram showing CS, DO, DI, CLK signals and data transfers.](/assets/posts/patreon/Pasted%20image%2020230124035040.png)
...
A criação de um plugin Sigrok é um processo simples, graças ao guia oficial detalhado disponível no site do Sigrok em [https://sigrok.org/wiki/Protocol\_decoder\_HOWTO](https://sigrok.org/wiki/Protocol_decoder_HOWTO). Dado que estamos trabalhando com o protocolo Serial Peripheral Interface (SPI), é adequado utilizar a funcionalidade “Decodificador de Pilha” do Sigrok, que permite encadear vários decodificadores e usar a saída de um decodificador como entrada de outro. Essa abordagem simplifica o processo, eliminando a necessidade de localizar e extrair manualmente bits individuais de dados dos dados brutos do SPI e permite que o foco seja nos bytes formados reais produzidos pelo decodificador SPI do Sigrok.
...
Esses são os arquivos necessários para o decodificador que foi desenvolvido. Basicamente, foi criado um decodificador para os registradores TPM, conforme descrito em [https://trustedcomputinggroup.org/wp-content/uploads/TCG\_PC\_Client\_Platform\_TPM\_Profile\_PTP\_2.0\_r1.03\_v22.pdf](https://trustedcomputinggroup.org/wp-content/uploads/TCG_PC_Client_Platform_TPM_Profile_PTP_2.0_r1.03_v22.pdf)

Após criar e salvar o projeto do PulseView, é necessário fechar e abrir novamente. O plugin deve ser carregado automaticamente. Na configuração do plugin SPI, na opção “Decodificador de Pilha”, a opção “TPM2.0” deverá estar disponível. ![SPI configuration panel with dropdown list including TPM2.0 and other device options.](/assets/posts/patreon/Pasted%20image%2020230124035119.png)E depois de selecionar, ele começará a tentar encontrar mensagens TPM2.0 sobre os dados SPI. ![SPI bus timing diagram showing CS, DO, DI, CLK signals and TPM2.0 data transfers.](/assets/posts/patreon/Pasted%20image%2020230124035129.png)
...
Para facilitar o uso, o decodificador anexado imprime as chaves encontradas no terminal de decodificação (para que não seja necessário usar a interface do PulseView, se não desejado) e também mostra em uma linha separada no PulseView. ![SPI communication waveform showing CS, DO, DI, CLK signals and TPM2.0 data transfers.](/assets/posts/patreon/Pasted%20image%2020230124035206.png)
...
Lucas Teske. “TPM 2.0: Extrair chaves do Bitlocker por SPI.” _Lets Hack It_, 2024. [https://lucasteske.dev/pt/2024/01/tpm-2-extraindo-chaves-bitlocker](https://lucasteske.dev/pt/2024/01/tpm-2-extraindo-chaves-bitlocker).

### BibTeX

```
@misc{teske2024tpm2extraindochavesbitlocker,
  author = {Lucas Teske},
