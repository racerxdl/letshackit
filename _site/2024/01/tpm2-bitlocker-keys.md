---
title: "TPM 2.0: Extracting Bitlocker keys through SPI | Lets Hack It"
description: "Learn how to extract BitLocker keys from TPM 2.0 using SPI bus sniffing. A guide to reverse engineering hardware security with logic analyzers."
image: "https://lucasteske.dev/assets/gepeto/tpm2.0.jpg"
---

# TPM 2.0: Extracting Bitlocker keys through SPI

 ![TPM 2.0: Extracting Bitlocker keys through SPI](/assets/gepeto/tpm2.0.jpg)Written by Lucas Teske   
on 16 January 2024
...
For this article, the assumptions and analyses are based on the 2.0 specification, version 1.03v22 of the TPM (as of the current article date, the most used specification in home computers and servers), which is available here: [https://trustedcomputinggroup.org/wp-content/uploads/TCG\_PC\_Client\_Platform\_TPM\_Profile\_PTP\_2.0\_r1.03\_v22.pdf](https://trustedcomputinggroup.org/wp-content/uploads/TCG_PC_Client_Platform_TPM_Profile_PTP_2.0_r1.03_v22.pdf)
...
The Trusted Platform Module (TPM) chip is usually located on a motherboard in a way that is not easily accessible, for example, on the keyboard side of a laptop. However, since the Serial Peripheral Interface (SPI) is a bus protocol, it can be inferred that all SPI devices on the motherboard use the same clock and data lines. As a result, it is possible to connect a logic analyzer to the SPI flash memory storing the BIOS/UEFI code and monitor any SPI transactions that occur when the flash memory is inactive (when the chip select signal for the flash memory is low). This approach allows for detecting the relevant transactions to recover Bitlocker keys. ![Close-up of a Winbond 25Q128JVSQ 1830 chip mounted on a blue circuit board.](/assets/posts/patreon/Pasted%20image%2020230124034954.png) ![Diagram showing BIOS and TPM components with signal lines labeled CS, DO, DI, CLK.](/assets/posts/patreon/Pasted%20image%2020230124035004.png)
...
In pulseview, we can analyze the SPI bytes using the SPI decoder function. ![Red screwdriver icon, two hexagonal shapes, and a function graph icon in a UI interface.](/assets/posts/patreon/Pasted%20image%2020230124035010.png) ![SPI decoder selector interface highlighting "Serial Peripheral Interface" option in a list.](/assets/posts/patreon/Pasted%20image%2020230124035020.png)

Next, we need to set up the SPI decoder for the correct signals to be able to see the decoded bytes. ![SPI configuration panel with green color, showing clock, data, and chip-select settings.](/assets/posts/patreon/Pasted%20image%2020230124035029.png)

Then, you will notice that, as we leave the CS# polarity field as “ **active-low** ”, it is decoding only the BIOS Flash transactions, not the “rest” of the bus. ![SPI bus timing diagram showing CS, DO, DI, CLK signals and MISO/MOSI data transfers.](/assets/posts/patreon/Pasted%20image%2020230124035036.png)

If we select “ **active-high** ”, we will analyze anything that is **not** BIOS flash (which is what we want): ![SPI bus timing diagram showing CS, DO, DI, CLK signals and data transfers.](/assets/posts/patreon/Pasted%20image%2020230124035040.png)
...
Creating a Sigrok plugin is a straightforward process, thanks to the detailed official guide available on the Sigrok website at [https://sigrok.org/wiki/Protocol\_decoder\_HOWTO](https://sigrok.org/wiki/Protocol_decoder_HOWTO). Given that we are working with the Serial Peripheral Interface (SPI) protocol, it is appropriate to use Sigrok’s “Stacked Decoder” functionality, which allows chaining multiple decoders and using the output of one decoder as input for another. This approach simplifies the process, eliminating the need to manually locate and extract individual bits of data from the raw SPI data and allows focusing on the actual formed bytes produced by the Sigrok SPI decoder.
...
These are the necessary files for the decoder that was developed. Basically, a decoder for the TPM registers was created, as described in [https://trustedcomputinggroup.org/wp-content/uploads/TCG\_PC\_Client\_Platform\_TPM\_Profile\_PTP\_2.0\_r1.03\_v22.pdf](https://trustedcomputinggroup.org/wp-content/uploads/TCG_PC_Client_Platform_TPM_Profile_PTP_2.0_r1.03_v22.pdf)

After creating and saving the PulseView project, you need to close and reopen it. The plugin should be loaded automatically. In the SPI plugin settings, in the “Stacked Decoder” option, the “TPM2.0” option should be available. ![SPI configuration panel with dropdown list including TPM2.0 and other device options.](/assets/posts/patreon/Pasted%20image%2020230124035119.png)And after selecting, it will start to try to find TPM2.0 messages over the SPI data. ![SPI bus timing diagram showing CS, DO, DI, CLK signals and TPM2.0 data transfers.](/assets/posts/patreon/Pasted%20image%2020230124035129.png)
...
To facilitate use, the attached decoder prints the found keys in the decoding terminal (so it is not necessary to use the PulseView interface, if not desired) and also shows in a separate line in PulseView. ![SPI communication waveform showing CS, DO, DI, CLK signals and TPM2.0 data transfers.](/assets/posts/patreon/Pasted%20image%2020230124035206.png)
...
All Consolidated References Here are all the consolidated references for you to check:

- TPM 2.0 Specification - [https://trustedcomputinggroup.org/wp-content/uploads/TCG\_PC\_Client\_Platform\_TPM\_Profile\_PTP\_2.0\_r1.03\_v22.pdf](https://trustedcomputinggroup.org/wp-content/uploads/TCG_PC_Client_Platform_TPM_Profile_PTP_2.0_r1.03_v22.pdf)
...
Lucas Teske. “TPM 2.0: Extracting Bitlocker keys through SPI.” _Lets Hack It_, 2024. [https://lucasteske.dev/2024/01/tpm2-bitlocker-keys](https://lucasteske.dev/2024/01/tpm2-bitlocker-keys).

### BibTeX

```
@misc{teske2024tpm2bitlockerkeys,
  author = {Lucas Teske},
  title = {TPM 2.0: Extracting Bitlocker keys through SPI},
  year = {2024},
  publisher = {Lets Hack It},
  url = {https://lucasteske.dev/2024/01/tpm2-bitlocker-keys}
}
```
