---
title: "Hacking a ESP32 into FPGA Board | Lets Hack It"
description: "Learn how to hack a Colorlight FPGA board by soldering an ESP32 and using it as a networked JTAG debugger for open-source tools."
image: "https://lucasteske.dev/assets/posts/medium/1_ALnLx9L06FSciqeEV5OCuQ.png"
---

# Hacking a ESP32 into FPGA Board

 ![Hacking a ESP32 into FPGA Board](/assets/posts/medium/1_ALnLx9L06FSciqeEV5OCuQ.png)Written by Lucas Teske   
on 14 June 2020

Colorlight Hub 5A-75B V6.1 Board

Last year I saw a russian guy that found out that this cheap board (US$15~) had an Lattice ECP5 FPGA, which is compatible with Open Source Tool-chains for synthesis. He was running a RISC-V Core inside that and piping the serial through the ethernet ports. I wanted to get one and start playing by myself. These boards are relatively cheap, about US$15 and contains a Lattice ECP5 FPGA ( LFE5U-25F-6BG381C ), 4MB DRAM, Two Gigabit Ethernet and several level shifters for I/O. This is good because:
...
I didn’t have to reverse engineer the board, since someone already did all the reverse engineering: [https://github.com/q3k/chubby75/blob/master/5a-75b/hardware\_V6.1.md](https://github.com/q3k/chubby75/blob/master/5a-75b/hardware_V6.1.md)
...
And the generated vhdl\_blink-flash.svf was persisting into the flash memory.

> After few days I managed to write thr SPI Flash using FT232H. Full [#opensource](https://twitter.com/hashtag/opensource?src=hash&ref_src=twsrc%5Etfw) stuff to program that board. Also only US$15. [pic.twitter.com/RujsOGwH1D](https://t.co/RujsOGwH1D)
> 
> — Cybernetic Lover (@lucasteske) [June 4, 2020](https://twitter.com/lucasteske/status/1268616857583419393?ref_src=twsrc%5Etfw)
...
![U23 and J4 removed](/assets/posts/medium/0_wK9QPXyVChE4n8py.jpeg)_U23 and J4 removed_
...
![Bypass wires soldered in U23](/assets/posts/medium/1_tT_gWmR6KPWXKKoewW-KDQ.png)_Bypass wires soldered in U23_
...
![Hot-glue raw pieces](/assets/posts/medium/1_53DN-rC8MrNRfFdVmzN-Uw.png)_Hot-glue raw pieces_
...
![Heat gun with hot-glue](/assets/posts/medium/1_7O77I22KdFxtNz4-G2cSeA.png)_Heat gun with hot-glue_

After cooling down, the results look really good:

![Hot-glue on bypass wires](/assets/posts/medium/1_sCDf7DO_2mY2FoOTLN357Q.png)_Hot-glue on bypass wires_
...
![Power Connector and Serial Port](/assets/posts/medium/1_sADN_JL_MLf9TP5r7FVymQ.png)_Power Connector and Serial Port_
...
![ESP32 fixed with double-sided table and wires soldered](/assets/posts/medium/1_86Ir5XIqvHCLbMsbWxs5MA.png)_ESP32 fixed with double-sided table and wires soldered_
...
![JTAG Power Header](/assets/posts/medium/1_Ltxy34-3hHoAalgDOHqG7A.png)_JTAG Power Header_

![GND wire soldered](/assets/posts/medium/1_Sae7vuGGrvCUuYZdOiGPVA.png)_GND wire soldered_
...
So yesterday someone sent me a link that was not related to that, but I found this lib: [**Lib(X)SVF - A library for implementing SVF and XSVF JTAG players** _JTAG (IEEE 1149.1, aka “Boundary Scan”) is a standard IC testing, debugging and programming port. SVF (Serial Vector…_www.clifford.at](http://www.clifford.at/libxsvf/)
...
> FINALLY. Now I can use the ESP32 as JTAG programmer for Lattice FPGA. Soon I will able to program through wifi and pipe a serial debug port as well. [pic.twitter.com/zs41v47BvU](https://t.co/zs41v47BvU)
> 
> — Cybernetic Lover (@lucasteske) [June 14, 2020](https://twitter.com/lucasteske/status/1272019368617095173?ref_src=twsrc%5Etfw)

I also added a simple command to switch the ESP32 from programming mode to Serial Passthrough. This way, after programming it can pipe all FPGA Serial Calls to the ESP32 serial. Then I did this small Serial Hello World for testing it: [**racerxdl/fpga-serial-hello**](https://github.com/racerxdl/fpga-serial-hello)

Which basically keeps sending Hello World through the serial port forever. And it does work!

![Hello World output from FPGA](/assets/posts/medium/1_Z8RdW5i7NQh7KUEC5v5NFg.png)_Hello World output from FPGA_
...
Lucas Teske. “Hacking a ESP32 into FPGA Board.” _Lets Hack It_, 2020. [https://lucasteske.dev/2020/06/hacking-a-esp32-into-fpga-board](https://lucasteske.dev/2020/06/hacking-a-esp32-into-fpga-board).

### BibTeX

```
@misc{teske2020hackingaesp32intofpgaboard,
  author = {Lucas Teske},
  title = {Hacking a ESP32 into FPGA Board},
  year = {2020},
  publisher = {Lets Hack It},
  url = {https://lucasteske.dev/2020/06/hacking-a-esp32-into-fpga-board}
}
```
