---
title: "Creating your own GSM Network with LimeSDR | Lets Hack It"
description: "Build a private GSM network with LimeSDR. Learn to install Osmocom, configure OpenBSC, and set up a Network in a Box."
image: "https://lucasteske.dev/assets/posts/medium/1_KbNtmYsmaCMVJinWnJdhFg.png"
---

# Creating your own GSM Network with LimeSDR

 ![Creating your own GSM Network with LimeSDR](/assets/posts/medium/1_KbNtmYsmaCMVJinWnJdhFg.png)Written by Lucas Teske   
on 21 December 2019
...
There are few files that need to be created. Let’s first start with the OpenBSC config file _openbsc.cfg_:
...
The only important parameter here to take care is **band**. Make sure is the same as in _openbsc.cfg_ file. The next one is _osmo-trx.cfg_ which will be used by _osmo-trx-lms:_
...
There are not much to change here. If you’re using a multi-port LimeSDR (like LimeSDR USB or PCIe) you can change the parameter _tx-path_ and \*rx-path \*to the desired paths.
...
![OSMO TRX LMS running](/assets/posts/medium/1_KbNtmYsmaCMVJinWnJdhFg.png)
...
![Sysmocom Custom Simcard [http://shop.sysmocom.de/products/sysmousim-sjs1-4ff](http://shop.sysmocom.de/products/sysmousim-sjs1-4ff)](/assets/posts/medium/0_pKv_npHA_3DhtIQi.jpg)_Sysmocom Custom Simcard [http://shop.sysmocom.de/products/sysmousim-sjs1-4ff](http://shop.sysmocom.de/products/sysmousim-sjs1-4ff)_

But since we set the BTS to _accept-all_ mode, you can just go to your phone network settings and select the created network. That will work fine if you have an Android phone which allows you to select custom networks:

![List of networks in an Android Phone. Here the 72470 network I just created](/assets/posts/medium/1_8_tklU4IBXyxERiyDkxl-w.png)_List of networks in an Android Phone. Here the 72470 network I just created_
...
![HUEBRNetwork showing in the list](/assets/posts/medium/1_CrDQk7YysiFXzgaOrxaTQA.png)_HUEBRNetwork showing in the list_
...
There are two scripts I found in the internet to send SMS. They basically selects the IMSI from the sqlite database the Osmo stack creates and then connects through the telnet interface to issue the desired commands. One of them is _sms\_broadcast.py_:
...
This will send an SMS to **all** connected devices as it was the _source number_.
...
Lucas Teske. “Creating your own GSM Network with LimeSDR.” _Lets Hack It_, 2019. [https://lucasteske.dev/2019/12/creating-your-own-gsm-network-with-limesdr/](https://lucasteske.dev/2019/12/creating-your-own-gsm-network-with-limesdr/).

### BibTeX

```
@misc{teske2019creatingyourowngsmnetworkwithlimesdr,
  author = {Lucas Teske},
